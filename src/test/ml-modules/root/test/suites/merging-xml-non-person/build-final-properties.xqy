...
let $orgname-map :=
  ...
  return $map
(: Both docs have the same value for the AnnualRevenue property. :)
let $annual-revenue-map :=
  for $map in $actual
  where map:contains(-$map, "AnnualRevenue")
  return $map
...
let $id-maps :=
  ...
  return $map
return (
  test:assert-exists($orgname-map),
  let $p := <OrgName>Global Corp</OrgName>
  let $actual := map:get($orgname-map[1], "values")
  return
    test:assert-equal-xml($p, $actual),

  test:assert-exists($annual-revenue-map),
  test:assert-equal(2, fn:count(map:get($annual-revenue-map, "sources"))),
  test:assert-equal(<AnnualRevenue>1287.9</AnnualRevenue>, map:get($annual-revenue-map, "values")),
