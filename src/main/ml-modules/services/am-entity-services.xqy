...
) as document-node()*
{
  ...
  document {
    array-node {
      let $entities := sem:sparql("SELECT ?entityIRI ?entityTitle
                                  WHERE
                                  {
      ...
      return object-node {
        ...
        "properties": array-node {
           let $properties := sem:sparql("SELECT ?propertyIRI ?datatype ?collation ?items ?title ?itemsDatatype ?itemsRef
                                          WHERE
                                          {
